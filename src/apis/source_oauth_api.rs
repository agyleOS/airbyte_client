...
/// struct for typed errors of method [`revoke_source_o_auth_tokens`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RevokeSourceOAuthTokensError {
...
) -> Result<crate::models::CompleteOAuthResponse, Error<CompleteSourceOAuthError>> {
...
pub async fn revoke_source_o_auth_tokens(
    configuration: &configuration::Configuration,
    revoke_source_oauth_tokens_request: crate::models::RevokeSourceOauthTokensRequest,
) -> Result<(), Error<RevokeSourceOAuthTokensError>> {
    let local_var_configuration = configuration;
    ...
    local_var_req_builder = local_var_req_builder.json(&revoke_source_oauth_tokens_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RevokeSourceOAuthTokensError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
